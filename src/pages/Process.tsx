...
export function ProcessPage({ lang }: { lang: "en" | "tr" }) {
  ...
  return (
    <div className="flex flex-col min-h-screen pt-20">
      ...
      {/* 1. HERO SECTION */}
      <section className="relative px-6 py-24 md:py-32 lg:py-40 bg-cream overflow-hidden">
        {/* Soft Ambient Background Gradients */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-pink/5 blur-[120px] rounded-full pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-purple/5 blur-[150px] rounded-full pointer-events-none"></div>
        
        <div className="mx-auto max-w-5xl text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-dark/5 border border-dark/5 mb-8">
              <div className="w-2 h-2 rounded-full bg-pink animate-pulse"></div>
              <span className="text-[11px] font-bold text-dark/70 tracking-widest uppercase">
                {lang === 'en' ? 'HOW UXIPEK WORKS' : 'UXİPEK NASIL ÇALIŞIR?'}
              </span>
            </div>

            <h1 className="text-4xl md:text-6xl lg:text-7xl font-medium text-dark mb-8 tracking-tight leading-[1.1]">
              {lang === 'en' ? '“Clarity before pixels.' : '“Önce netlik, sonra tasarım.'}
              <br className="hidden md:block" />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink to-purple font-semibold">
            ...
            <div className="flex items-center justify-center">
              <Link 
                to={`/${lang}/contact`} 
                className="inline-flex items-center justify-center px-10 py-5 text-sm font-bold text-white transition-all rounded-full bg-pink shadow-lg shadow-pink/20 hover:-translate-y-1 hover:shadow-xl hover:bg-pink/90"
              >
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <motion.div 
              ...
              className="bg-[#F8F7F4] p-8 md:p-12 rounded-[2rem] border border-dark/5 flex flex-col gap-6"
            >
              <div className="w-16 h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center text-pink">
      ...
      <section className="px-6 py-24 md:py-32 bg-[#F8F7F4] relative border-t border-dark/5">
          ...
          <div className="flex flex-col md:flex-row gap-12 lg:gap-24 items-center">
                ...
                <ul className="space-y-6">
                  {[
                    ...
                    <motion.li 
                      ...
                      className="flex items-center gap-4 text-dark/80 font-medium text-lg"
                    >
                      <div className="w-6 h-6 rounded-full bg-pink/10 flex items-center justify-center shrink-0">
                        <CheckCircle2 size={14} className="text-pink" strokeWidth={3} />
                      </div>
                      {item}
                    </motion.li>
                  ))}
            ...
            <div className="flex-1 w-full">
              <motion.div 
                ...
                className="bg-white p-8 md:p-12 rounded-[2rem] border border-dark/5 shadow-xl shadow-dark/5 relative overflow-hidden"
              >
                {/* Decorative element */}
                <div className="absolute top-0 right-0 w-32 h-32 bg-pink/5 blur-3xl rounded-full"></div>
                <MessageSquare className="w-12 h-12 text-pink mb-8 opacity-80" strokeWidth={1} />
                <div className="space-y-4">
                  <div className="h-2 bg-dark/5 rounded-full w-3/4"></div>
                  <div className="h-2 bg-dark/5 rounded-full w-full"></div>
                  <div className="h-2 bg-dark/5 rounded-full w-5/6"></div>
                  <div className="h-2 bg-pink/20 rounded-full w-1/2 mt-8"></div>
                </div>
              </motion.div>
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link 
                to={`/${lang}/contact`} 
                className="w-full sm:w-auto inline-flex items-center justify-center px-10 py-5 text-sm font-bold text-white transition-all rounded-full bg-pink shadow-lg shadow-pink/20 hover:-translate-y-1 hover:shadow-xl hover:bg-pink/90"
              >
                {lang === 'en' ? 'Start a Project' : 'Projenizi Başlatalım'}
              </Link>
              <Link 
